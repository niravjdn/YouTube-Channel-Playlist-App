...
public class PagerAdapterPlaylist extends FragmentStatePagerAdapter {
    int mNumOfTabs;

    public PagerAdapterPlaylist(FragmentManager fm, int NumOfTabs) {
        super(fm);
    ...
    public Fragment getItem(int position) {
        ...
        if(position>=0){
            if(position< TabModel.ptabnameList.size()){
                if(TabModel.pplaylistOrChannelList.get(position).equalsIgnoreCase("playlist")){
                    String tabUrl = TabModel.pidRegardingyoutubeList.get(position);
                    Log.d("App: positiontaburl",position+""+tabUrl);
                    PlayListFragment playTab = PlayListFragment.newInstance(tabUrl);
                    return playTab;
                }else if(TabModel.pplaylistOrChannelList.get(position).equalsIgnoreCase("playlistSplash")){
                    String tabUrl = TabModel.pidRegardingyoutubeList.get(position);
                    Log.d("App: positiontaburl",position+""+tabUrl);
