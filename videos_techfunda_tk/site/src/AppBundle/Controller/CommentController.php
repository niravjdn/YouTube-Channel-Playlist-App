...
class CommentController extends Controller
{
    ...
    public function api_listAction($id,$token)
    {
        ...
        $comments=array();
        if ($video!=null) {
            $comments=$em->getRepository('AppBundle:Comment')->findBy(array("video"=>$video));
        }
    ...
    public function hide_twoAction($id,Request $request){
        ...
        if($comment==null){
        	...
        	$id_article=$comment->getVideo()->getId();
        	if ($comment->getEnabled()==true) {
        		$comment->setEnabled(false);
        	}else{
        		$comment->setEnabled(true);
    ...
    public function deleteAction($id,Request $request){
        ...
        if ($request->query->has("video")) {
            $url = $this->generateUrl('app_video_view',array("id"=>$video->getId()));
        }
