...
public class PagerAdapterBhajan extends FragmentStatePagerAdapter {
    int mNumOfTabs;

    public PagerAdapterBhajan(FragmentManager fm, int NumOfTabs) {
        super(fm);
    ...
    public Fragment getItem(int position) {
        ...
        if(position>=0){
            if(position< TabModel.bhajanTabnameList.size()){
                if(TabModel.bhajanPlaylistOrChannelList.get(position).equalsIgnoreCase("playlist")){
                    String tabUrl = TabModel.bhajanIdRegardingyoutubeList.get(position);
