...
public class PagerAdapterNatak extends FragmentStatePagerAdapter {
    int mNumOfTabs;

    public PagerAdapterNatak(FragmentManager fm, int NumOfTabs) {
        super(fm);
    ...
    public Fragment getItem(int position) {
        ...
        if(position>=0){
            if(position< TabModel.natakTabnameList.size()){
                if(TabModel.natakPlaylistOrChannelList.get(position).equalsIgnoreCase("playlist")){
                    String tabUrl = TabModel.natakIdRegardingyoutubeList.get(position);
