...
public class PagerAdapterSpeech extends FragmentStatePagerAdapter {
    int mNumOfTabs;

    public PagerAdapterSpeech(FragmentManager fm, int NumOfTabs) {
        super(fm);
    ...
    public Fragment getItem(int position) {
        ...
        if(position>=0){
            if(position< TabModel.speechtabnameList.size()){
                if(TabModel.speechPlaylistOrChannelList.get(position).equalsIgnoreCase("playlist")){
                    String tabUrl = TabModel.speechIdRegardingyoutubeList.get(position);
