...
public class PagerAdapterGarba extends FragmentStatePagerAdapter {
    int mNumOfTabs;

    public PagerAdapterGarba(FragmentManager fm, int NumOfTabs) {
        super(fm);
    ...
    public Fragment getItem(int position) {
        ...
        if(position>=0){
            if(position< TabModel.garbaIdRegardingyoutubeList.size()){
                if(TabModel.garbaPlaylistOrChannelList.get(position).equalsIgnoreCase("playlist")){
                    String tabUrl = TabModel.garbaIdRegardingyoutubeList.get(position);
